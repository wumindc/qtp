...
import { GATEWAY_PORT, CONTEXT_PATH, getLocalServiceUrl } from '@ai-quality-platform/shared-config';
...
export class HealthController {
  ...
  @Get('health.do')
  async health() {
    const [platform, execution, aiInvocation] = await Promise.all([
      this.probeInternalService('system', '/health.do'),
      this.probeInternalService('execution', '/health.do'),
      this.probeAiInvocationService('/health.do'),
    ]);
    const aggregateStatus: ServiceHealthStatus = platform.status === 'UP' && execution.status === 'UP' && aiInvocation.status === 'UP' ? 'UP' : 'DOWN';
  ...
  private async probeInternalService(segment: 'system' | 'execution', path: string): Promise<InternalServiceHealth> {
    return this.probeUrl(buildGatewayTargetUrl(segment, path));
  }

  private async probeAiInvocationService(path: string): Promise<InternalServiceHealth> {
    const normalizedPath = path.startsWith('/') ? path : `/${path}`;
    return this.probeUrl(`${getLocalServiceUrl('aiInvocation')}/${CONTEXT_PATH}${normalizedPath}`);
  }

  private async probeUrl(url: string): Promise<InternalServiceHealth> {
