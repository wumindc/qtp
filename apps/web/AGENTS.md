...
### 响应式与移动端自适应（Responsive Design）

- **优先流式布局**：所有页面布局应优先采用流式设计（`flex`, `grid`），避免在容器或分栏上写死绝对宽度（如 `w-[800px]`）。
- **组件降级折叠**：
  - 在移动端（`< md`），如果水平空间不足，应使用 `flex-col` 将内容上下堆叠排列。
  - 对于全局侧栏或较长的分类列表，在移动端需通过**抽屉（Sheet/Drawer）**或**横向滚动视图（overflow-x-auto）**来进行交互降级，严禁挤压主内容区。
  - 复杂操作按钮组在移动端应折叠入“更多（DropdownMenu）”下拉菜单中。
- **数据溢出保护**：表格或较宽的数据展示模块，必须在其外层包裹 `<div className="w-full overflow-x-auto">`，确保在小屏幕下不会撑破全局布局。
